...
void Effects::update(float delta_time)
{
    ...
    case TREMBLE:
        float max_value = 0.050f;
        float random_float_x = ((float)rand() / RAND_MAX) * (0.0f - max_value) + max_value;
        float random_float_y = ((float)rand() / RAND_MAX) * (0.0f - max_value) + max_value;
        std::cout << random_float_x << " " << random_float_y << std::endl;
        tremble_offset = glm::vec3(random_float_x, random_float_y, 1.0f);
        break;
    }
}
